...
final class DemoDrive: XCTestCase {
    ...
    func tap(_ label: String, in query: XCUIElementQuery? = nil,
        ...
        // .firstMatch, because a label is not unique: "Import kubeconfig" is
        // both at the foot of the sidebar and in the middle of Overview's
        // empty state, and an ambiguous query throws rather than picking.
    ...
    ///
    /// Clicking the row opens it: its screens appear beneath it in the sidebar,
    /// and that is where the `goTo` that follows finds them.
    ...
    func beat2Import() throws {
        // Import is at the foot of the sidebar now, so there is no walk to a
        // screen first: the take opens on the empty Overview and the click is
    ...
    /// Three clusters, probed. Health is the point, and the list of clusters
    /// is the sidebar now — so this rests on Overview, which ranks them by it.
    ...
    /// The closing shot: back to Overview, all three in.
